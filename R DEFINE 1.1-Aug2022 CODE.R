...
rm(list=ls(all=T))
T<-80 #number of years
loops<-12#number of scenarios that are run (minimum 6)
...
monte_sce<-1 #number of Monte Carlo simulations for scenarios 2 and 3
...
Y_star_N_Exp<- vector(length=T)
g_star_N<- vector(length=T)
...
con_change<- vector(length=T)
g_L_per<- vector(length=T)
...
for (j in 1:loops){
  ...
  assign(paste("Monte_g_L_per",sce, sep=""), matrix(nrow=T,ncol=monte))
  ...
  assign(paste("Monte_EMIS_F_Y",sce, sep=""), matrix(nrow=T,ncol=monte)) 
  assign(paste("Monte_EMIS_F",sce, sep=""), matrix(nrow=T,ncol=monte)) 
  ...
  assign(paste("Monte_TEMP",sce, sep=""), matrix(nrow=T,ncol=monte)) 
  ...
  assign(paste("Monte_s_W",sce, sep=""), matrix(nrow=T,ncol=monte)) 
  
  assign(paste("g_Y_per_mean",sce, sep=""), vector(length=T))
  assign(paste("g_L_per_mean",sce, sep=""), vector(length=T))
  ...
  assign(paste("EMIS_F_Y_mean",sce, sep=""), vector(length=T))
  assign(paste("EMIS_F_mean",sce, sep=""), vector(length=T))
  ...
  assign(paste("TEMP_mean",sce, sep=""), vector(length=T))
  ...
  assign(paste("s_W_mean",sce, sep=""), vector(length=T))
  ...
  for (k in 1:monte) {
    GVA_S1<-4.6 #trillion US$ 
    GVA_S2<-18.7#trillion US$ 
    GVA_S3<-7.7#trillion US$  
    GVA_S4<-52.7#trillion US$ 
    GVA<- GVA_S1+GVA_S2+GVA_S3+GVA_S4# trillion US$
    carbon_S1<-15.89#GtCO2 
    carbon_S2<-6.12#GtCO2 
    carbon_S3<-8.22#GtCO2 
    ...
    sh_EMIS_F_S1<-carbon_S1/(carbon_S1+carbon_S2+carbon_S3+carbon_S4)
    sh_EMIS_F_S2<- carbon_S2/(carbon_S1+carbon_S2+carbon_S3+carbon_S4)
    sh_EMIS_F_S3<- carbon_S3/(carbon_S1+carbon_S2+carbon_S3+carbon_S4)
    sh_EMIS_F_S4<- 1-sh_EMIS_F_S1-sh_EMIS_F_S2-sh_EMIS_F_S3
    ...
    B_G_initial<-0.45#trillion US$ 
    B_initial<-14.4 #trillion US$ 
    B_C_initial<-B_initial-B_G_initial #auxiliary exogenous variable
    B_CB_initial<-0.5 #0.5tr is the estimated amount of corporate sector holdings 
    B_GCB_initial<-0.08*B_CB_initial #trillion US$; 0.08 is the proportion of green bonds in the total corporate banks held by central banks
    s_G<-B_GCB_initial/(B_G_initial/(1+g_baseline))
    B_CCB_initial<- B_CB_initial -B_GCB_initial 
    s_C<-B_CCB_initial/(B_C_initial/(1+g_baseline))
    prop<-0.75
    
    #$0.2tr is the assumed non-energy related investment in 2018 which is allocated among the sectors based on their GVA
    #$0.342tr was the energy investment in 2020
    #$0.027tr was the industry and buildings & infrastructure investment in 2020
    #$0.177tr was the agriculture and information energy-related investment in 2020
    #$0.5tr was the green energy-related investment in 2020
    #$0.5-0.328-0.122-0.009=0.041tr was the residual green energy-related investment in 2020 which is allocated among the sectors based on their GVA
    
    IG_NE_initial<-0.2
    IG_S1_E_initial<-0.342+sh_GVA_S1*0.021
    IG_S2_E_initial<-0.027+sh_GVA_S1*0.021
    IG_S3_E_initial<-0.177+sh_GVA_S1*0.021
    IG_S4_E_initial<-0.009+sh_GVA_S1*0.021
    IG_initial<-IG_S1_E_initial+IG_S2_E_initial+IG_S3_E_initial+IG_S4_E_initial+IG_NE_initial
    
    IG_PRI_initial<- prop*IG_initial #trillion US$
    IG_GOV_initial<-(1-prop)*IG_initial #trillion US$
    Y_initial<-96.10#trillion US  
    w_G_initial<-1  
    w_C_S1_initial<-1 
    w_C_S2_initial<-1 
    w_C_S3_initial<-1 
    ...
    w_C_S3_2022<-1 
    w_C_S4_2022<-1 
    r_3_dCR_dCAR_max<- -1.39308039 # econometrically estimated coefficient
    l_1<-1 
    spr_1<- 0.03371429 #econometrically estimated coefficient
    spr_3<-1
    beta_2<-1 
    gov_IG<-IG_GOV_initial/(Y_initial/(1+g_baseline))
    dummy_con_change<-0 #auxiliary parameter
    con_change[1]<-0
    con_change[80]<-0.15
    duration<-80-3
    tau_C_dummy_H<-0
    ...
    for (i in 1:T) {
      ...
      #####################################################
      #Medium carbon tax without recycling of carbon tax revenues
      #####################################################
      if (j==4 & i<4){}
      if (j==4 & i>=4){
      ...
      #Sufficiency
      ##########
      if (j==10 & i<4 && i >4+duration) {}
      if (j==10 & i>=4 && i <=4+duration) {
        dummy_con_change<-1 
        phi<-0.25
      }
      ######################################
      #Green fiscal policy mix+GSF+DPF+Green QE
      ######################################
      if (j==11 & i<4) {}
      if (j==11 & i>=4) {
        w_G_2022<-1-0.25 
      ...
      if (i == 1) {
        for (iterations in 1:15){
          ...
          ucr_dummy<-1
          cr_adjust<-0.013*cr_rationing_dummy #auxiliary parameter
          dam_adjust<-0.0015*damage_dummy #auxiliary parameter
          stag_adjust<-0.2*stagnation_dummy #auxiliary parameter
          ############
          ...
          haz<-0.04  
          con_M<-0.0015
          xi<-(g_baseline*(DEM[i]-mu[i]*delta[i]*(K[i]/(1+g_baseline))))/(CO_PRI[i]*mu[i]-DEM[i]+mu[i]*delta[i]*(K[i]/(1+g_baseline))) #ensures that the growth rate of DC is equal to g baseline
          ...
          #2.1.3 Emissions and climate change
          f_nc<-0.14
          car<-3.667 #auxiliary parameter
          TCRE<-0.44/1000#0C/GtCO2;0.44 is TCRE measured in 0C/1000GtCO2
          
          #2.1.4 Ecological efficiency and technology
          zeta_1<-0.0015
          g_omega_initial<--0.001 #auxiliary parameter
          mu_max<-1.5 
          mu_min<-0.3 
          mu_rat<-0.9 #auxiliary parameter
          mu_2049<-mu_rat*mu[i] #auxiliary parameter
          rho_max<-0.8 
          rho_rat<-1.39 #auxiliary parameter
          rho_2049<-rho_rat*rho[i] #auxiliary parameter
          epsilon_max<-12 
          epsilon_min<-2 
          epsilon_rat<-0.71 #auxiliary parameter
          ...
          KGE_KCE_2049<-0.0872#auxiliary parameter
          KGNE_KCNE_2049<-0.0320#auxiliary parameter
          KSEQ_KCE_2049<-0.0013#auxiliary parameter
          pi_2<-(log((mu[i]-mu_min)/(mu_max-mu[i]))-log((mu_2049-mu_min)/(mu_max-mu_2049)))/(KGNE_KCNE_2049-1*(KGNE[i])/(KCNE[i])) 
          pi_1<-((mu[i]-mu_min)/(mu_max-mu[i]))*exp(pi_2*1*(KGNE[i])/(KCNE[i])) 
          pi_4<-(log(rho_max-rho[i])-log(rho_max-rho_2049)-log(rho[i])+log(rho_2049))/(KGNE_KCNE_2049-1*(KGNE[i])/(KCNE[i])) 
          pi_3<-((rho_max-rho[i])*exp(pi_4*1*(KGNE[i])/(KCNE[i])))/rho[i] 
          pi_6<-(log((epsilon[i]-epsilon_min)/(epsilon_max-epsilon[i]))-log((epsilon_2049-epsilon_min)/(epsilon_max-epsilon_2049)))/(KGE_KCE_2049-1*(KGE[i])/(KCE[i])) 
          pi_5<-((epsilon[i]-epsilon_min)/(epsilon_max-epsilon[i]))*exp(pi_6*1*(KGE[i])/(KCE[i])) 
          pi_8<-(log(1-theta[i])-log(1-theta_2049)-log(theta[i])+log(theta_2049))/(KGE_KCE_2049-1*(KGE[i])/(KCE[i]))
          pi_7<-((1-theta[i])*exp(pi_8*1*(KGE[i])/(KCE[i])))/theta[i] 
          pi_10<-(log(1-seq[i])-log(1-seq_2049)-log(seq[i])+log(seq_2049))/(KSEQ_KCE_2049-1*(KSEQ[i])/(KCE_PRI_S1[i]+KCE_PRI_S2[i]))
          pi_9<-((1-seq[i])*exp(pi_10*1*(KSEQ[i])/(KCE_PRI_S1[i]+KCE_PRI_S2[i])))/seq[i] 
          ...
          #2.2.2 Firms
          alpha_00<-(g_baseline+cr_adjust+dam_adjust+delta[1])*2 #stabilises I/K
          alpha_1_dI_du_max<-0.069#econometrically estimated coefficient
          alpha_1<-(alpha_1_dI_du_max*4)/(alpha_00+2*delta[1])
          alpha_2_dI_dr_max<-0.077#econometrically estimated coefficient 
          alpha_2<-(alpha_2_dI_dr_max*4)/(alpha_00+2*delta[1]) 
          alpha_31_dI_dur_max<-0.004#econometrically estimated coefficient
          alpha_31<-(alpha_31_dI_dur_max*4)/(alpha_00+2*delta[1])
          alpha_32<-0.5 
          alpha_41<-0.1 
          alpha_42<-0.99 
          alpha_51<-0.1 
          alpha_52<-0.99 
          delta_0<-0.048
          K_Y_ratio<-(0.26*(1+g_baseline))/(g_baseline+delta_0) #auxiliary parameter
          s_W[i]<- 0.55
          s_W_1<-0.184#econometrically estimated coefficient
          s_W_0<-s_W[i]+s_W_1*ur[i]
          hours<-1893 
          beta_1<-17 
          
          tau_F<-0.15 
          x_11<-0.25 
          x_10<-x_1[i]+x_11*yield_C[i]  
          x_21<-0.25 
          x_20[i]<-x_2[i]+x_21*yield_G[i] 
          coupon_C[i]<-yield_C[i]*p_C[i] 
          coupon_G[i]<-yield_G[i]*p_G[i] 
          p_C_bar<-100 
          p_G_bar<-100 
          g_beta_initial<-0.0002 #auxiliary parameter
          zeta_2<-0.5 
          beta_initial<-IG_PRI_D[i]/I_PRI_D[i] #auxiliary parameter; stabilises KG(PRI)/K(PRI)
          beta_initial_S1<-IG_PRI_D_S1[i]/I_PRI_D_S1[i] #auxiliary parameter
          beta_initial_S2<-IG_PRI_D_S2[i]/I_PRI_D_S2[i] #auxiliary parameter
          beta_initial_S3<-IG_PRI_D_S3[i]/I_PRI_D_S3[i] #auxiliary parameter
          beta_initial_S4<-IG_PRI_D_S4[i]/I_PRI_D_S4[i] #auxiliary parameter
          zeta_3<-0.008
          sigma_0_initial<- -0.01636*(1/(1-zeta_3)) #auxiliary parameter
          sigma_1<-0.01 
          sigma_2<-0.850  #econometrically estimated  parameter
          g_x_20[i]<-0.02
          zeta_4<-0.4 
          endog[i]<-1
          zeta_7<- 0.7
          ...
          sh_GREEN_S1<-(IG_S1_E_initial +sh_GVA_S1*0.2)/IG_initial 
          sh_GREEN_S2<- (IG_S2_E_initial +sh_GVA_S2*0.2)/IG_initial 
          sh_GREEN_S3<-(IG_S3_E_initial +sh_GVA_S3*0.2)/IG_initial 
          sh_GREEN_S4<-(IG_S4_E_initial +sh_GVA_S4*0.2)/IG_initial
          gamma_E1<-( IG_S1_E_initial *prop)/IG_PRI_S1[i]
          gamma_E2<- (IG_S2_E_initial *prop)/IG_PRI_S2[i]
          gamma_E3<-( IG_S3_E_initial *prop)/IG_PRI_S3[i]
          gamma_E4<-( IG_S4_E_initial *prop)/IG_PRI_S4[i]
          
          gamma_E<-IG_E[i]/IG_initial
          gamma_SEQ1<-(0.6*2.3/1000)/IG_PRI_S1[i] #we assume that 60% of the sequestration investment in in the power sector 
          gamma_SEQ2<-(0.4*2.3/1000)/IG_PRI_S2[i] #we assume that 40% of the sequestration investment in in the manufacturing and construction sector
          
          #2.2.3 Households 
          c_2<-0.047 #econometrically estimated parameter
          pr<-0.99 
          lambda_12<- -0.01 
          lambda_13<- -0.01 
          lambda_14<- -0.01 
          ...
          lambda_34<- -0.01 
          lambda_35<- -0.01 
          portfolio_dam<-0.2 #auxiliary parameter
          lambda_10prime<- 0.5*portfolio_dam 
          lambda_20prime<- -1*portfolio_dam 
          lambda_30prime<- 0*portfolio_dam 
          lambda_40prime<- -lambda_10prime-lambda_20prime-lambda_30prime 
          lambda_21<- lambda_12 
          lambda_41<- lambda_14 
          lambda_42<- lambda_24 
          lambda_43<- lambda_34 
          lambda_31<- lambda_13 
          lambda_32<- lambda_23 
          lambda_11<- -lambda_21- lambda_31-lambda_41 
          lambda_22<- -lambda_12-lambda_32-lambda_42 
          lambda_33<- -lambda_13-lambda_23-lambda_43 
          lambda_44<- -lambda_14-lambda_24-lambda_34 
          lambda_45<- -lambda_15-lambda_25-lambda_35 
          int_A<-0.056
          int_D<-0.04
          int_S<-0.0175
          zeta_5<-0.0145 
          zeta_6<-0.0003 
          lf_2<-0.001 
          lf_1[i]<-LF[i]/(POP[i]*(1-(1-ad_LF)*D_TF[i]))+lf_2*hazratio[i]
          PORT_BCH[i]<-B_CH[i]/ (V_HF[i]/(1+g_baseline)) #auxiliary parameter
          PORT_BGH[i]<-B_GH[i]/(V_HF[i]/(1+g_baseline)) #auxiliary parameter
          PORT_SECH[i]<-SEC_H[i]/(V_HF[i]/(1+g_baseline)) #auxiliary parameter
          PORT_D[i]<-1- PORT_BCH[i]-PORT_BGH[i]- PORT_SECH[i]  #auxiliary parameter
          lambda_10<-(SEC_H[i]/(V_HF[i]/(1+g_baseline)))-lambda_10prime*D_T[i]-lambda_11*int_S-lambda_12*yield_C[i]-lambda_13*yield_G[i]-lambda_14*int_D-lambda_15*(Y_H[i]/V_HF[i]) 
          lambda_20<-(B_CH[i]/(V_HF[i]/(1+g_baseline)))-lambda_20prime*D_T[i]-lambda_21*int_S-lambda_22*yield_C[i]-lambda_23*yield_G[i]-lambda_24*int_D-lambda_25*(Y_H[i]/V_HF[i]) 
          lambda_30[i]<-(B_GH[i]/(V_HF[i]/(1+g_baseline)))-lambda_30prime*D_T[i]-lambda_31*int_S-lambda_32*yield_C[i]-lambda_33*yield_G[i]-lambda_34*int_D-lambda_35*(Y_H[i]/V_HF[i]) 
          lambda_40[i]<-1- lambda_10- lambda_20- lambda_30[i] 
          zeta_10<-0.18 
          ...
          h_1<-0.21
          def_max<-0.2 
          def_0<-(def_max/def[i])-1 
          def_2_ddef_dilliq_max<-0.25 #auxiliary parameter 
          def_2<-(def_2_ddef_dilliq_max)*((1+def_0)^2)/(def_max*def_0) 
          def_1<-def_2*illiq[i] 
          CR_max<-0.5 
          CAR_min<-0.08
          lev_B_max<-1/0.03
          CR_initial<-0.2 #auxiliary parameter
          h_2<-SEC_B[i]/D[i] 
          w_H<-0 
          w_S<-0 
          ...
          spr_initial<- 0.05-(int_A-int_D) #0.05 is the spread between loans and deposit interest rate taken from World Bank
          spr_2<- 0.02325 #econometrically estimated parameter 
          ...
          c_1<-((Y[i]*g_baseline)/K_PRI[i]-(c_2*V_HF[i])/K_PRI[i]-g_baseline-delta[i]-(Y[i]*gov_C)/K_PRI[i] -(Y[i]*gov_IC)/K_PRI[i] -(Y[i]*gov_IG)/K_PRI[i]+Y[i]/K_PRI[i])/(Y_H[i]/K_PRI[i]) #stabilises Y/K; 
          ...
          tau_H<-(gov_C+gov_IC+gov_IG-(tau_F*TP_G[i])/Y[i] -(tau_C[i]*EMIS_F[i])/Y[i]+(int_S*SEC[i])/Y[i]-(coupon_C[i]*b_CCB[i]+coupon_G[i]*b_GCB[i]+int_A*A[i]+int_S*SEC_CB[i])/Y[i]+SUB[i]*(1+g_baseline)/Y[i]-(SEC[i]*g_baseline)/Y[i])/(Y_HG[i]/Y[i]) #stabilises SEC/Y
          
          s_B<-(def[i]*L[i]+CAP[i]*g_baseline)/BP[i] #stabilises CAP/K
          
          V_CB[i]<-0 #stabilises V_CB/K
          
          g_bC[i]<-(g_baseline-g_pC[i])/(1+ g_pC[i]) #stabilises the growth rate of B_C
          g_bG[i]<-(g_baseline-g_pG[i])/(1+ g_pG[i]) #stabilises the growth rate of B_G
          g_pC[i]<- 0.0 
          g_pG[i]<-0.0 
          
          x_1[i]<-(B_C[i]*g_bC[i])/(IC_PRI_D[i]*(1+g_bC[i])) #stabilises the growth rate of b_C
          x_2[i]<-(B_G[i]*g_bG[i])/(IG_PRI_D[i]*(1+g_bG[i])) #stabilises the growth rate of b_G
          
          I_PRI[i]<-prop*0.26*Y[i]#trillion US$ 
          ...
          alpha_01<-alpha_1*u[i]+alpha_2*r[i]-alpha_31*ur[i]^(-alpha_32)-alpha_41*(1-ue[i])^(-alpha_42)-alpha_51*(1-um[i])^(-alpha_52) #stabilises I/K
          ...
          W[i]<-DEM[i]-REC[i]#Gt
          CEN[i]<-(1/car)*EMIS_F[i] #Gt
          O2[i]<-EMIS_F[i]-CEN[i] #Gt
          HW_CUM[i]<- 16#Gt 
          ...
          g_EMIS_L[i]<-0.02 
          EMIS_L[i]<- 2.9# GtCO2 
          EMIS[i]<- EMIS_F[i]+EMIS_L[i] # GtCO2
          TEMP[i]<-1.21 #0C  
          CO2_CUM[i]<-(TEMP[i]*(1-f_nc))/TCRE # GtCO2
          
          #2.1.4 Ecological efficiency and technology
          omega[i]<-EMIS_F[i]/(E_F[i]*(1-seq[i])) #GtCO2/EJ
          g_omega[i]<-g_omega_initial
          mu[i]<- MY[i]/(Y[i]-CO_GOV[i]) #Gt/trillion $ or kg/$
          rho[i]<- REC[i]/DEM[i]  
          epsilon[i]<-E[i]/Y[i] #EJ/trillion $
          theta[i]<- 0.15
          seq[i]<-149.3/(1000*EMIS_F[i]) #149.3Mt is the value of sequestrated emissions in  2021
          ...
          Y_star_N_Exp[i]<-Y_star_N[i] #trillion US$
          g_star_N[i]<-g_baseline
          Y_star[i]<-min(min(Y_star_M[i], Y_star_E[i]),min(Y_star_N_Exp[i], Y_star_K[i])) #trillion US$
          Y[i]<-Y_initial#trillion US$
          um[i]<- (Y[i]-CO_GOV[i])/Y_star_M[i]
          ue[i]<-Y[i]/Y_star_E[i] 
          u[i]<-0.73
          re[i]<-1- ur[i] 
          D_T[i]<-(1-(1/(1+eta_1*TEMP[i]+eta_2*TEMP[i]^2+eta_3*TEMP[i]^6.754)))*damage_dummy
          D_TP[i]<-p*D_T[i]
          ...
          beta_0_S1[i]<-beta_S1[i]+beta_1*sh_EMIS_F_S1*(tucr[i]/(1-g_ucr[i])-tucn[i]/(1+g_ucn[i]))+beta_2*(sh_L[i]*(int_G[i]-int_C_S1[i])+(1-sh_L[i])*(yield_G[i]-yield_C[i]))
          beta_0_S2[i]<- beta_S2[i]+beta_1*sh_EMIS_F_S2*( tucr[i]/(1-g_ucr[i])-tucn[i]/(1+g_ucn[i]))+beta_2*(sh_L[i]*(int_G[i]-int_C_S2[i])+(1-sh_L[i])*(yield_G[i]-yield_C[i]))
          beta_0_S3[i]<-beta_S3[i]+beta_1*sh_EMIS_F_S3*( tucr[i]/(1-g_ucr[i])-tucn[i]/(1+g_ucn[i]))+beta_2*(sh_L[i]*(int_G[i]-int_C_S3[i])+(1-sh_L[i])*(yield_G[i]-yield_C[i])) 
          beta_0_S4[i]<-beta_S4[i]+beta_1*sh_EMIS_F_S4*( tucr[i]/(1-g_ucr[i])-tucn[i]/(1+g_ucn[i]))+beta_2*(sh_L[i]*(int_G[i]-int_C_S4[i])+(1-sh_L[i])*(yield_G[i]-yield_C[i]))
          ...
          L[i]<-(1.01-B[i]/Y[i])*Y[i] #101.0 is the credit to non-financial corporations/GDP ratio; trillion US$ 
          ...
          wage[i]<-s_W[i]*lambda[i]*h[i] #trillion US$/millions of employees
          ...
          SEC_B[i]<-0.16*SEC[i] #trillion US$ 
          ...
          r_0<-(CR_max/CR_initial)-1 
          r_2_dCR_ddsr_max<- 1.02476498 #econometrically estimated coefficient 
          r_2<-(r_2_dCR_ddsr_max *((1+r_0)^2))/(CR_max*r_0) 
          r_3<- -(r_3_dCR_dCAR_max *((1+r_0)^2))/(CR_max*r_0) 
          r_1<-r_2*dsr[i]-r_3*(CAR[i]-CAR_min)
          sh_NLG[i]<- 0.03036948#NLG_D[i]/(NLG_D[i]+NLC_D_S1[i]+NLC_D_S2[i] +NLC_D_S3[i]+NLC_D_S4[i])
          sh_NLC_S1[i]<- 0.05328913#NLC_D_S1[i]/(NLG_D_S1[i]+NLG_D_S2[i]+NLG_D_S3[i]+NLG_D_S4[i]+NLC_D_S1[i]+NLC_D_S2[i] +NLC_D_S3[i]+NLC_D_S4[i])
          sh_NLC_S2[i]<- 0.21663191#NLC_D_S2[i]/(NLG_D_S1[i]+NLG_D_S2[i]+NLG_D_S3[i]+NLG_D_S4[i]+NLC_D_S1[i]+NLC_D_S2[i] +NLC_D_S3[i]+NLC_D_S4[i])
          sh_NLC_S3[i]<- 0.08920137#NLC_D_S3[i]/(NLG_D_S1[i]+NLG_D_S2[i]+NLG_D_S3[i]+NLG_D_S4[i]+NLC_D_S1[i]+NLC_D_S2[i] +NLC_D_S3[i]+NLC_D_S4[i])
          ...
          SEC[i]<-0.97*Y[i] #trillion US$, 96.96 general government debt to GDP 
          IG_GOV[i]<-IG_GOV_initial#trillion US$
          IC_GOV[i]<-I_GOV[i]-IG_GOV[i] #trillion US$ 
          I_GOV[i]<- (1-prop)*0.26*Y[i] #trillion US$ 
          ...
          TAX_C[i]<- 0.084# trillion US$  
          tau_C[i]<- TAX_C[i]/EMIS_F[i]
          ...
          #Auxiliary variables
          g_Y[i]<- 0.058 #g_baseline 
          IG_E[i]<-IG_initial-IG_NE_initial#trillion US$
          ...
          g_L_per[i]<- g_baseline*100
          
          r_2_dCR_ddsr_max_pseudo<- -(-1.111*0.186)*(L[i]/(NLC_D_S1[i]+ NLC_D_S2[i]+ NLC_D_S3[i]+ NLC_D_S4[i]+ NLG_D_S1[i]+ NLG_D_S2[i]+ NLG_D_S3[i]+ NLG_D_S4[i]))# -1.111 is taken from the econometric estimates; 0.186 is the sum of the interest rate and repayment ratio 
          
          r_3_dCR_dCAR_max_pseudo<- -(0.524*(0.0839/0.1565))*(L[i]/(NLC_D_S1[i]+ NLC_D_S2[i]+ NLC_D_S3[i]+ NLC_D_S4[i]+ NLG_D_S1[i]+ NLG_D_S2[i]+ NLG_D_S3[i]+ NLG_D_S4[i]))# 0.527 is taken from the econometric estimates; 0.1565 is taken from IMF and is the average value of CAR for the period 1995-2019; 0.0839  is taken from OECD and is the average value of Capital/assets for the same period 
          
          spr_1_pseudo<- (-0.059*(0.0864/0.1512))# -0.059 is taken from the econometric estimates; 0.1512  is taken from IMF and is the average value of CAR for the period 1995-2019; 0.0864 is taken from OECD and is the average value of Capital/assets for the same period
          
          spr_2_pseudo <- (0.125*0.186)# 0.125 is taken from the econometric estimates; 0.186 is the sum of the interest rate and repayment ratio
          
          EMIS_F_Y[i]<- EMIS_F[i]/Y[i]
          EMIS_F_Y_SSP360[i]<- Data[1,c(4)]
        }
      }
      ...
      ####################
        ...
        for (iterations in 1:15){
          ...
          W[i]<-M[i]+CEN[i]+O2[i]-EMIS_F[i]-(SES[i]- SES[i-1])  #Eq. (6)
          CEN[i]<-(1/car)*EMIS_F[i] #Eq. (7)
          O2[i]<-EMIS_F[i]-CEN[i] #Eq. (8)
          ...
          Y_star[i]<-min(min(Y_star_M[i], Y_star_E[i]),min(Y_star_N_Exp[i], Y_star_K[i])) #Eq. (40)
          ...
          D_T[i]<-(1-(1/(1+eta_1*TEMP[i]+eta_2*TEMP[i]^2+eta_3*TEMP[i]^6.754)))*damage_dummy #Eq. (46)
          ...
          I_PRI_D[i]<-(((alpha_00)/(1+exp(random1[i]*random_dummy+alpha_01-alpha_1*u[i-1]-alpha_2*r[i-1]+(alpha_31*ur[i-1]^(-alpha_32))+(alpha_41*(1-ue[i-1])^(-alpha_42))+(alpha_51*(1-um[i-1])^(-alpha_52))))-delta[1]+delta[i])*K_PRI[i-1])*(1-D_T[i-1])*(1-stag_adjust*g_POP_stag[i]) #Eq. (54)
          ...
          beta_S1[i]<-min(beta[i-1]*(1-beta_is_endogenous_different)+(beta_0_S1[i]-beta_1*sh_EMIS_F_S1* (tucr[i-1]-tucn[i-1])-beta_2*(sh_L[i-1]*(int_G[i-1]-int_C_S1[i-1])+(1-sh_L[i-1])*(yield_G[i-1]-yield_C[i-1])))* beta_is_endogenous_different,1) #Eq. (57)
          beta_S2[i]<- min(beta[i-1]* (1-beta_is_endogenous_different)+(beta_0_S2[i]-beta_1*sh_EMIS_F_S2*( tucr[i-1]-tucn[i-1])-beta_2*(sh_L[i-1]*(int_G[i-1]-int_C_S2[i-1])+(1-sh_L[i-1])*(yield_G[i-1]-yield_C[i-1])))*beta_is_endogenous_different ,1)#Eq. (57)
          beta_S3[i]<-min(beta[i-1]*(1-beta_is_endogenous_different)+(beta_0_S3[i]-beta_1*sh_EMIS_F_S3*( tucr[i-1]-tucn[i-1])-beta_2*(sh_L[i-1]*(int_G[i-1]-int_C_S3[i-1])+(1-sh_L[i-1])*(yield_G[i-1]-yield_C[i-1])))*beta_is_endogenous_different,1)#Eq. (57)
          beta_S4[i]<- min(beta[i-1]*(1-beta_is_endogenous_different)+(beta_0_S4[i]-beta_1*sh_EMIS_F_S4*( tucr[i-1]-tucn[i-1])-beta_2*(sh_L[i-1]*(int_G[i-1]-int_C_S4[i-1])+(1-sh_L[i-1])*(yield_G[i-1]-yield_C[i-1])))*beta_is_endogenous_different ,1)#Eq. (57)
          ...
          g_lambda[i]<-sigma_0[i]+sigma_1+sigma_2*g_Y[i]#Eq. (94)
          ...
          wage[i]<-s_W[i]*lambda[i]*h[i] #Eq. (97)
          s_W[i]<-s_W_0-s_W_1*ur[i-1] #Eq. (98)
          N[i]<-Y[i]/(lambda[i] *h[i])  #Eq. (99)	
          ur[i]<-1-re[i] #Eq. (100)
          h[i]<-h[i-1]*(1+g_h[i]) #Eq. (101)
          g_h[i]<-min(0, phi*( ur_T-ur[i-1])) #Eq. (102)
          b_C[i]<-b_C[i-1]+(x_1[i]*IC_PRI_D[i])/p_C_bar #Eq. (103)
          b_G[i]<-b_G[i-1]+(x_2[i]*IG_PRI_D[i])/p_G_bar #Eq. (104) 
          x_1[i]<-max(x_10-x_11*yield_C[i-1],0) #Eq. (105)  
          x_2[i]<-x_20[i]-x_21*yield_G[i-1]    #Eq. (106)
          x_20[i]<-x_20[i-1]*(1+g_x_20[i]) #Eq. (107)
          g_x_20[i]<-g_x_20[i-1]*(1-zeta_4) #Eq. (108)
          yield_C[i]<-coupon_C[i]/p_C[i] #Eq. (109)
          yield_G[i]<-coupon_G[i]/p_G[i] #Eq. (110)
          coupon_C[i]<-yield_C[i-1]*p_C_bar #Eq. (111)
          coupon_G[i]<-yield_G[i-1]*p_G_bar #Eq. (112)
          B_C[i]<-B_CH[i]+B_CCB[i]  #Eq. (113)
          B_G[i]<-B_GH[i]+B_GCB[i]  #Eq. (114)
          p_C[i]<-B_C[i]/b_C[i] #Eq. (115)
          p_G[i]<-B_G[i]/b_G[i]  #Eq. (116)
          B[i]<-B_C[i]+B_G[i] #Eq. (117)
          DL[i]<-def[i]*L[i-1] #Eq. (118)
          def[i]<-def_max/(1+def_0*exp(def_1-def_2*illiq[i-1])) #Eq. (119)
          
          illiq[i]<-((int_C_S1[i-1]+rep)*LC_S1[i-1]+(int_C_S2[i-1]+rep)*LC_S2[i-1]+(int_C_S3[i-1]+rep)*LC_S3[i-1]+(int_C_S4[i-1]+rep)*LC_S4[i-1]+(int_G[i-1]+rep)*LG[i-1]+coupon_C[i-1]*b_C[i-1]+coupon_G[i-1]*b_G[i-1]+wage[i]*N[i]+TAX_F[i]+TAX_C[i]-SUB[i]+delta[i]*K_PRI[i-1])/(Y[i]+(1-CR_C_S1[i])*NLC_D_S1[i]+(1-CR_C_S2[i])*NLC_D_S2[i]+(1-CR_C_S3[i])*NLC_D_S3[i]+(1-CR_C_S4[i])*NLC_D_S4[i]+ (1-CR_G[i])*(NLG_D_S1[i]+ NLG_D_S2[i]+ NLG_D_S3[i]+NLG_D_S4[i])+(b_C[i]-b_C[i-1])*p_C_bar+ (b_G[i]-b_G[i-1])*p_G_bar)  #Eq. (120)
          
          dsr[i]<-((int_C_S1[i-1]+rep)*LC_S1[i-1]+(int_C_S2[i-1]+rep)*LC_S2[i-1]+(int_C_S3[i-1]+rep)*LC_S3[i-1]+(int_C_S4[i-1]+rep)*LC_S4[i-1]+(int_G[i-1]+rep)*LG[i-1]+coupon_C[i-1]*b_C[i-1]+coupon_G[i-1]*b_G[i-1])/(TP[i]+int_C_S1[i-1]*LC_S1[i-1]+int_C_S2[i-1]*LC_S2[i-1]+int_C_S3[i-1]*LC_S3[i-1]+int_C_S4[i-1]*LC_S4[i-1]+(int_G[i-1])*LG[i-1]+coupon_C[i-1]*b_C[i-1]+coupon_G[i-1]*b_G[i-1]) #Eq. (121)
          
          #2.2.3 Households
          Y_HG[i]<-wage[i]*N[i]+DP[i]+BP_D[i]+int_D*D[i-1]+int_S*SEC_H[i-1]+coupon_C[i-1]*b_CH[i-1]+coupon_G[i-1]*b_GH[i-1]  #Eq. (122)
          Y_H[i]<-Y_HG[i]-TAX_H[i] #Eq. (123)
          con_change[i]<-con_change[i-1]+(con_change[80]/duration)*dummy_con_change
          CO_PRI_N[i]<-((c_1-con_change[i]*c_1)*Y_H[i-1]+(c_2-con_change[i]*c_2)*V_HF[i-1])*(1-D_T[i-1]) #Eq. (124)
          if (Y_N[i]<Y_star[i]) {CO_PRI[i]<-CO_PRI_N[i]} else {CO_PRI[i]<- pr*(Y_star[i]-CO_GOV[i]-I_GOV[i]-I_PRI[i])} #Eq. (125)
          V_HF[i]<-V_HF[i-1]+Y_H[i]-CO_PRI[i]+b_CH[i-1]*(p_C[i]-p_C[i-1])+b_GH[i-1]*(p_G[i]-p_G[i-1])#Eq. (126)
          SEC_H[i]<-(lambda_10+lambda_10prime*D_T[i-1]+lambda_11*int_S+lambda_12*yield_C[i-1]+lambda_13*yield_G[i-1]+lambda_14*int_D+lambda_15*(Y_H[i-1]/V_HF[i-1]))*V_HF[i-1] #Eq. (127)
          B_CH[i]<-(lambda_20+lambda_20prime*D_T[i-1]+lambda_21*int_S+lambda_22*yield_C[i-1]+lambda_23*yield_G[i-1]+lambda_24*int_D+lambda_25*(Y_H[i-1]/V_HF[i-1]))*V_HF[i-1] #Eq. (128)
          B_GH[i]<-(lambda_30[i]+lambda_30prime*D_T[i-1]+lambda_31*int_S+lambda_32*yield_C[i-1]+lambda_33*yield_G[i-1]+lambda_34*int_D+lambda_35*(Y_H[i-1]/V_HF[i-1]))*V_HF[i-1] #Eq. (129)
          D_N[i]<-(lambda_40[i]+lambda_40prime*D_T[i-1]+lambda_41*int_S+lambda_42*yield_C[i-1]+lambda_43*yield_G[i-1]+lambda_44*int_D+lambda_45*(Y_H[i-1]/V_HF[i-1]))*V_HF[i-1] #Eq. (130n)
          D[i]<-D[i-1]+Y_H[i]-CO_PRI[i]-(SEC_H[i]-SEC_H[i-1])-(b_CH[i]-b_CH[i-1])*p_C_bar-(b_GH[i]-b_GH[i-1])*p_G_bar  #Eq. (130)
          lambda_30[i]<-lambda_30[i-1]*(1+g_lambda_30[i]) #Eq. (131)
          g_lambda_30[i]<-zeta_10*g_bG[i-1] #Eq. (132)
          lambda_40[i]<-1- lambda_10- lambda_20- lambda_30[i]  
          b_CH[i]<-B_CH[i]/p_C[i] #Eq. (133)
          b_GH[i]<-B_GH[i]/p_G[i] #Eq. (134)
          DC[i]<-DC[i-1]+CO_PRI[i]-xi*DC[i-1] #Eq. (135)
          g_POP[i]<-g_POP[i-1]*(1-zeta_5) #Eq. (136)
          POP[i]<-POP[i-1]*(1+g_POP[i]) #Eq. (137)
          LF[i]<-(lf_1[i]-lf_2*hazratio[i-1])*(1-(1-ad_LF)*D_TF[i-1])*POP[i] #Eq. (138)
          lf_1[i]<-lf_1[i-1]*(1-zeta_6) #Eq. (139)
          
          #2.2.4 Banks
          BP[i]<-int_C_S1[i-1]*LC_S1[i-1]+int_C_S2[i-1]*LC_S2[i-1] +int_C_S3[i-1]*LC_S3[i-1] +int_C_S4[i-1]*LC_S4[i-1]+int_G[i-1]*(LG_S1[i-1]+LG_S2[i-1]+LG_S3[i-1]+LG_S4[i-1])+int_S*SEC_B[i-1]-int_D*D[i-1]-int_A*A[i-1] #Eq. (140)
          CAP[i]<-CAP[i-1]+BP_U[i]-DL[i]+BAILOUT[i] #Eq. (141)
          CAP_before[i]<-CAP[i-1]+BP_U[i]-DL[i]
          BP_U[i]<-s_B*BP[i-1] #Eq. (142)
          BP_D[i]<-BP[i]-BP_U[i] #Eq. (143)
          HPM[i]<-h_1*D[i] #Eq. (144)
          SEC_BN[i]<-SEC_B[i-1]+(A[i]-A[i-1])+(D[i]-D[i-1])+BP_U[i]+BAILOUT[i] -(LG[i]-LG[i-1])-(LC[i]-LC[i-1])-(HPM[i]-HPM[i-1])-DL[i] 
          if (iterations>10 & A_N[i]>0) {SEC_B[i]<-h_2*D[i]} else {SEC_B[i]<-SEC_BN[i]} #Eq. (145)
          A_N[i]<-A[i-1]+(h_2*D[i]-SEC_B[i-1])+(LG[i]-LG[i-1])+(LC[i]-LC[i-1])+(HPM[i]-HPM[i-1])+DL[i]-(D[i]-D[i-1])-BP_U[i]-BAILOUT[i]   
          if (iterations>10 & A_N[i]>0) {A[i]<-A_N[i]} else {A[i]<-0} #Eq. (146)
          
          if (i<4) {w_G[i]<-w_G_initial} else {w_G[i]<-w_G_2022} 
          if (i<4) {w_C_S1[i]<-w_C_S1_initial} else {w_C_S1[i]<-w_C_S1_2022} 
          if (i<4) {w_C_S2[i]<-w_C_S2_initial} else {w_C_S2[i]<-w_C_S2_2022} 
          if (i<4) {w_C_S3[i]<-w_C_S3_initial} else {w_C_S3[i]<-w_C_S3_2022} 
          if (i<4) {w_C_S4[i]<-w_C_S4_initial} else {w_C_S4[i]<-w_C_S4_2022} 
          
          CR[i]<-CR_max/(1+r_0*exp(r_1-r_2*dsr[i-1]+r_3*(CAR[i-1]-CAR_min)))*cr_rationing_dummy#Eq. (147)
          CR_G[i]<- (1+l_1*(w_G[i-1]-w_LT[i-1]))*CR[i] #Eq. (148)
          CR_C_S1[i]<-(1+l_1*(w_C_S1[i-1]-w_LT[i-1]))*CR[i] #Eq. (149)
          CR_C_S2[i]<-(1+l_1*(w_C_S2[i-1]-w_LT[i-1]))*CR[i] #Eq. (149)
          CR_C_S3[i]<-(1+l_1*(w_C_S3[i-1]-w_LT[i-1]))*CR[i]  #Eq. (149)
          CR_C_S4[i]<-(CR[i]-sh_NLG[i-1]*CR_G[i]-sh_NLC_S1[i-1]*CR_C_S1[i]-sh_NLC_S2[i-1]*CR_C_S2[i]-sh_NLC_S3[i-1]*CR_C_S3[i])/(sh_NLC_S4[i-1]+0.001)  #0.001 has been used in order to allow for CR_C_S4[i[ to be defined even if sh_NLC_4 is zero; #Eq. (150) 
          
          LC_S1[i]<-LC_S1[i-1]+(1-CR_C_S1[i])*NLC_D_S1[i]-rep*LC_S1[i-1]-def[i]*LC_S1[i-1] #Eq. (151)
          LC_S2[i]<-LC_S2[i-1]+(1-CR_C_S2[i])*NLC_D_S2[i]-rep*LC_S2[i-1]-def[i]*LC_S2[i-1] #Eq. (151)
          LC_S3[i]<-LC_S3[i-1]+(1-CR_C_S3[i])*NLC_D_S3[i]-rep*LC_S3[i-1]-def[i]*LC_S3[i-1] #Eq. (151)
          LC_S4[i]<-LC_S4[i-1]+(1-CR_C_S4[i])*NLC_D_S4[i]-rep*LC_S4[i-1]-def[i]*LC_S4[i-1] #Eq. (151)
          LG_S1[i]<-LG_S1[i-1]+(1-CR_G[i])*NLG_D_S1[i]-rep*LG_S1[i-1]-def[i]*LG_S1[i-1]  #Eq. (152)
          LG_S2[i]<-LG_S2[i-1]+(1-CR_G[i])*NLG_D_S2[i]-rep*LG_S2[i-1]-def[i]*LG_S2[i-1]  #Eq. (152)
          LG_S3[i]<-LG_S3[i-1]+(1-CR_G[i])*NLG_D_S3[i]-rep*LG_S3[i-1]-def[i]*LG_S3[i-1]  #Eq. (152)
          LG_S4[i]<-LG_S4[i-1]+(1-CR_G[i])*NLG_D_S4[i]-rep*LG_S4[i-1]-def[i]*LG_S4[i-1]  #Eq. (152)
          LC[i]<-LC_S1[i]+LC_S2[i]+LC_S3[i]+LC_S4[i] #Eq. (153)
          LG[i]<-LG_S1[i]+LG_S2[i]+LG_S3[i]+LG_S4[i] #Eq. (154)
          lev_B[i]<-(HPM[i]+LG[i]+LC[i]+SEC_B[i])/CAP[i] #Eq. (155)
          CAR[i]<-CAP[i]/(w_G[i]*LG[i]+w_C_S1[i]*LC_S1[i]+w_C_S2[i]*LC_S2[i]+w_C_S3[i]*LC_S3[i] +w_C_S4[i]*LC_S4[i]+w_S*SEC_B[i]+w_H*HPM[i]) #Eq. (156)
          ...
          w_LT[i]<-sh_LG[i-1]*w_G[i]+sh_LC_S1[i-1]*w_C_S1[i]+sh_LC_S2[i-1]*w_C_S2[i]+sh_LC_S3[i-1]*w_C_S3[i]+sh_LC_S4[i-1]*w_C_S4[i] #Eq. (157)
          int_G[i]<-spr_G[i]+int_A #Eq. (158)
          int_C_S1[i]<-spr_C_S1[i]+int_A #Eq. (159)
          int_C_S2[i]<-spr_C_S2[i]+int_A #Eq. (159)
          int_C_S3[i]<-spr_C_S3[i]+int_A #Eq. (159)
          int_C_S4[i]<-spr_C_S4[i]+int_A #Eq. (159)
          
          spr[i]<-spr_0-spr_1*(CAR[i-1]-CAR_min)+spr_2*dsr[i-1] #Eq. (160)
          spr_G[i]<-(1+spr_3*(w_G[i-1]-w_LT[i-1]))*spr[i] #Eq. (161)
          spr_C_S1[i]<-(1+spr_3*(w_C_S1[i-1]-w_LT[i-1]))*spr[i] #Eq. (162)
          spr_C_S2[i]<- (1+spr_3*(w_C_S2[i-1]-w_LT[i-1]))*spr[i]  #Eq. (162)
          spr_C_S3[i]<- (1+spr_3*(w_C_S3[i-1]-w_LT[i-1]))*spr[i]  #Eq. (162)
          spr_C_S4[i]<-(spr[i]-sh_LG[i-1]*spr_G[i]-sh_LC_S1[i-1]*spr_C_S1[i]-sh_LC_S2[i-1]*spr_C_S2[i]-sh_LC_S3[i-1]*spr_C_S3[i])/sh_LC_S4[i-1] #Eq. (163)
          ...
          #2.2.5 Government
          GNS[i]<- -CO_GOV[i]+TAX[i]-int_S*SEC[i-1]+CBP[i]-SUB[i]-delta[i]*K_GOV[i-1]#Eq. (164)
          SEC[i]<-SEC[i-1]+I_GOV[i]-GNS[i]-delta[i]*K_GOV[i-1]+BAILOUT[i]#Eq. (165)
          IG_GOV[i]<-gov_IG*Y[i-1] #Eq. (166)
          IC_GOV[i]<-gov_IC*Y[i-1] #Eq. (167)
          I_GOV[i]<- IC_GOV[i]+IG_GOV[i] #Eq. (168)
          KG_GOV[i]<-KG_GOV[i-1]+IG_GOV[i]-delta[i]*KG_GOV[i-1] #Eq. (169)
          KC_GOV[i]<-KC_GOV[i-1]+IC_GOV[i]-delta[i]*KC_GOV[i-1] #Eq. (170)
          K_GOV[i]<-KC_GOV[i]+KG_GOV[i] #Eq. (171)
          K[i]<-K_PRI[i]+K_GOV[i] #Eq. (172)
          KG[i]<-KG_PRI[i]+KG_GOV[i] #Eq. (173)
          KC[i]<-KC_PRI[i]+KC_GOV[i] #Eq. (174)
          ...
          CO_GOV[i]<-gov_C*Y[i-1] #Eq. (175)
          
          if (i<4) {SUB[i]<- tau_C[i]*EMIS_F[i-1]} else {SUB[i]<- (1-stop_recycling_dummy)* tau_C[i]*EMIS_F[i-1] + stop_recycling_dummy*((1-subsidy_increase_dummy)*tau_C[4]*EMIS_F[3]+subsidy_increase_dummy*gov_SUB[i]*E_NF[i-1]*ucr[i-1])} # Eq. (176)
          
          if (i<4){gov_SUB[i]<-min(SUB[i]/(E_NF[i-1]*ucr[i-1]),0.999)} else {gov_SUB[i]<-(1-subsidy_increase_dummy)*min(SUB[i]/(E_NF[i-1]*ucr[i-1]),0.999)+subsidy_increase_dummy*subsidy_ratio*gov_SUB[3]} # Eq. (177)
          
          TAX_H[i]<-tau_H*Y_HG[i-1] #Eq. (178)
          TAX_F[i]<-tau_F*TP_G[i-1] #Eq. (179)
          TAX_C[i]<-tau_C[i]*EMIS_F[i-1] #Eq. (180)
          
          if (i<4) {tau_C[i]<- tau_C_baseline[i]} else {tau_C[i]<- (1- tau_C_dummy_H - tau_C_dummy_M)*tau_C_baseline[i]+tau_C_dummy_H*tau_C_policy_High[i] +tau_C_dummy_M*tau_C_policy_Medium[i]} 
          tau_C_baseline[i]<-Data[i,c(1)]/1000
          tau_C_policy_Medium[i]<-Data[i,c(2)]/1000
          tau_C_policy_High[i]<-Data[i,c(3)]/1000
          TAX[i]<-TAX_H[i]+TAX_F[i]+TAX_C[i]    #Eq. (181)
          
          #2.2.6 Central bank
          CBP[i]<-coupon_C[i-1]*b_CCB[i-1]+coupon_G[i-1]*b_GCB[i-1]+int_A*A[i-1]+int_S*SEC_CB[i-1]  #Eq. (182)
          B_GCB[i]<-s_G*B_G[i-1] #Eq. (183)
          B_CCB[i]<-s_C*B_C[i-1] #Eq. (184)
          b_CCB[i]<-B_CCB[i]/p_C[i] #Eq. (185)
          b_GCB[i]<-B_GCB[i]/p_G[i] #Eq. (186)
          SEC_CB[i]<-SEC[i]-SEC_H[i]-SEC_B[i]  #Eq. (187)
          SEC_CBred[i]<-SEC_CB[i-1]+(HPM[i]-HPM[i-1])-(A[i]-A[i-1])-p_C_bar*(b_CCB[i]-b_CCB[i-1])-p_G_bar*(b_GCB[i]-b_GCB[i-1]) #Eq. (188-red)
          ...
          Y_H_K_ratio[i]<-Y_H[i]/K_PRI[i]
          A_K_ratio[i]<-A[i]/K_PRI[i]
          sigma_0_optimal[i]<-(((g_Y[i]-g_lf[i])/(1+ g_lf[i])*(1-(1-ad_P)*D_TP[i-1]))+(1-ad_P)*D_TP[i-1])/(1-(1-ad_P)*D_TP[i-1]) -sigma_1-sigma_2*g_Y[i]
          ...
          yield_C_per[i]<- yield_C[i]*100
          yield_G_per[i]<- yield_G[i]*100
          Y_star_N_Exp[i]<-Y_star_N[i-1]*(1+g_star_N[i-1]) 
          g_star_N[i]<-( Y_star_N[i]-Y_star_N[i-1])/ Y_star_N[i-1]
          ...
          EMIS_F_Y[i]<- EMIS_F[i]/Y[i]
          EMIS_F_Y_SSP360[i]<- Data[i,c(4)]
          g_L_per[i]<-(L[i]-L[i-1])/L[i-1]*100
    ...
    eval(parse(text=(paste("Monte_g_L_per",sce,"[,",k,"]","<-", "g_L_per", sep=""))))
    ...
    eval(parse(text=(paste("Monte_EMIS_F_Y",sce,"[,",k,"]","<-","EMIS_F_Y", sep=""))))
    eval(parse(text=(paste("Monte_EMIS_F",sce,"[,",k,"]","<-","EMIS_F", sep=""))))
    ...
    eval(parse(text=(paste("Monte_TEMP",sce,"[,",k,"]","<-","TEMP", sep=""))))
    ...
    eval(parse(text=(paste("Monte_s_W",sce,"[,",k,"]","<-","s_W", sep=""))))
    
    for (i in 1:T){
      
      eval(parse(text=(paste("g_Y_per_mean", sce, "[",i,"]", "<-", "mean(Monte_g_Y_per", sce, "[",i,",])", sep=""))))
      eval(parse(text=(paste("g_L_per_mean", sce, "[",i,"]", "<-", "mean(Monte_g_L_per", sce, "[",i,",])", sep=""))))
      ...
      eval(parse(text=(paste("EMIS_F_Y_mean", sce, "[",i,"]", "<-", "mean(Monte_EMIS_F_Y", sce, "[",i,",])", sep=""))))
      eval(parse(text=(paste("EMIS_F_mean", sce, "[",i,"]", "<-", "mean(Monte_EMIS_F", sce, "[",i,",])", sep=""))))
      ...
      eval(parse(text=(paste("TEMP_mean", sce, "[",i,"]", "<-", "mean(Monte_TEMP", sce, "[",i,",])", sep=""))))
      ...
      eval(parse(text=(paste("s_W_mean", sce, "[",i,"]", "<-", "mean(Monte_s_W", sce, "[",i,",])", sep=""))))
  ...
  if (sce<(scenarios_print+1)) {
    ...
    var3<-sprintf ("%.3f", g_Y[30])
    var4<-sprintf ("%.3f", ur[30])
    var5<-sprintf ("%.2f", POP[30])
    var6<-sprintf ("%.2f", lf[30])
    var7<-sprintf ("%.2f", omega_ratio[30])
    var8<-sprintf ("%.2f", mu_ratio[30])
    var9<-sprintf ("%.2f", epsilon_ratio[30])
    var10<-sprintf ("%.2f", rho_ratio[30])
    var11<-sprintf ("%.2f", theta[30])
    var12<-sprintf ("%.2f", theta[80])
    var13<-sprintf ("%.4f", KGE_KCE[29])
    var14<-sprintf ("%.4f", KGNE_KCNE[29])
    var15<-sprintf ("%.4f", KSEQ_KCE[29])
    var16<-sprintf ("%.5f", sigma_0_optimal[2])
    var17<-sprintf ("%.2f", IG_cum[30]/30)
    var18<-sprintf ("%.2f", IC_cum[30]/30) 
    var19<-sprintf ("%.2f", B_GCB[5])
    var20<-sprintf ("%.2f", E_ratio[30])
    var21<-sprintf ("%.3f", CO2_ratio[30])
    var22<-sprintf ("%.2f", TEMP[30])
    var23<-sprintf ("%.3f", (g_Y_cum[30])/30)
    var24<-sprintf ("%.3f", (TAX_C_change[5]))
    var25<-sprintf ("%.3f", (SUB_change[5]))
    var26<-sprintf ("%.3f", (IG_GOV_change[4]))
    var27<-sprintf ("%.3f", (seq[30]))
    var28<-sprintf ("%.3f", ((g_Y_cum[80])-g_Y_cum[30])/50)
    var29<-sprintf ("%.2f", epsilon_ratio[80])
    var30<-sprintf ("%.3f", (seq[80]))
    var31<-sprintf ("%.2f", E_ratio[80])
    var32<-sprintf ("%.3f", CO2_ratio[80])
    var33<-sprintf ("%.2f", TEMP[80])
    var34<-sprintf ("%.1f", POP[80])
    var35<-sprintf ("%.2f", omega_ratio[80])
    var36<-sprintf ("%.2f", (EMIS_L[30]/EMIS_L[1]))
    var37<-sprintf ("%.1f", EMIS_L[80])
    var38<-sprintf ("%.2f", IG_E_Y_cum[30]/30) 
    ...
    cat ("IG_annual(2021-2050):", var17," ")
    cat ("IC_annual(2021-2050):", var18," ")
    cat ("IG_E_Y_annual(2021-2050):", var38," ")
    cat ("\n")
    cat ("E_rat(2050):", var20," ")
    cat ("E_rat(2100):", var31," ")
    cat ("CO2_rat(2050):", var21," ")
    cat ("CO2_rat(2100):", var32," ")
    cat ("EMIS_L(2050/2021):", var36," ")
    cat ("EMIS_L(2100):", var37," ")
    cat ("TEMP(2050):", var22," ")
    cat ("TEMP(2100):", var33," ")
    cat ("\n")
    cat ("B_GCB(2050):", var19," ")
    cat ("TAX_C_change(2025):", var24," ")
    cat ("SUB_change(2025):", var25," ")
    cat ("IG_GOV_change(2024):", var26," ")
  ...
  assign (matrixname, (round(cbind(SEC_CB, SEC_CBred, A_K_ratio, HPM_K_ratio, D_K_ratio, A, A_N, HPM, D, V_HF, B_GCB, B_CCB, g_Y, gI, gC, g_K, gGOVCO, ID_K_ratio, I_K_ratio, I_PRI_D, I_PRI_D_S1, I_PRI_D_S2, I_PRI_D_S3, I_PRI_D_S4, I_PRI, I_PRI_S1, I_PRI_S2, I_PRI_S3, I_PRI_S4, SEC, SEC_H, SEC_B, SEC_Y, x_1, x_2, PORT_BCH, PORT_BGH, PORT_SECH, PORT_D, B, B_G, B_GH, B_C, B_CH, b_C, b_G, p_C, p_G, HPM, D, A, LC, LC_S1, LC_S2, LC_S3, LC_S4, LG, LG_S1, LG_S2, LG_S3, LG_S4, CAP, sh_NLG, sh_NLC_S1, sh_NLC_S2, sh_NLC_S3, sh_NLC_S4, CR, CR_C_S1, CR_C_S2, CR_C_S3, CR_C_S4, CR_G, dsr, def, illiq, L_K, I, lev_B, CAR, fiscal_balance, TAX, TAX_H, TAX_F, CO_GOV, D_T, D_TP, D_TF,  delta, LF, lf, POP, g_POP, um, ue, u, re, v, Y_POP_ratio, g_lambda, g_EN_ratio, g_YE_ratio, lambda, lambda_perworker, E_N_ratio, Y_E_ratio, CO_PRI, I_PRI, gI, theta, kappa, beta, beta_S1, beta_S2, beta_S3, beta_S4, Y, Wbill, Interest, Depreciation,  Y_N, Y_star, Y_star_M, Y_star_E, Y_star_N, Y_star_N_Exp, Y_star_K, Y_H, TP, RP, DP, L, K_PRI, KC_PRI, KC_PRI_S1, KC_PRI_S2, KC_PRI_S3, KC_PRI_S4, KG_PRI, KG_PRI_S1, KG_PRI_S2, KG_PRI_S3, KG_PRI_S4, L_K, r, r_total, IC_PRI_D, IC_PRI_D_S1, IC_PRI_D_S2, IC_PRI_D_S3, IC_PRI_D_S4, IC_PRI, IC_PRI_S1, IC_PRI_S2, IC_PRI_S3, IC_PRI_S4, IG_PRI_D, IG_PRI_D_S1, IG_PRI_D_S2, IG_PRI_D_S3, IG_PRI_D_S4,IG_PRI, IG_PRI_S1, IG_PRI_S2, IG_PRI_S3, IG_PRI_S4, NLC_D_S1, NLC_D_S2, NLC_D_S3, NLC_D_S4, NLG_D_S1, NLG_D_S2, NLG_D_S3, NLG_D_S4,  BP, N, Y_K_ratio, C_K_ratio, I_Y_ratio, Y_H_Y_ratio, Y_H_K_ratio, C_Y_ratio, sigma_0, sigma_0_optimal, h_optimal, h, g_h, g_Y_per, ur_per, greenI, p_C, yield_C,V_CB,  g_pC, g_bC, B_C_issue, p_G, yield_G, g_pG, g_bG, B_G_issue, IG_cum, IC_cum, w_G, w_C_S1, w_C_S2, w_C_S3, w_C_S4,BAILOUT, BAILOUT_CAR, BAILOUT_levB, int_G, int_C_S1, int_C_S2, int_C_S3, int_C_S4, spr_G, spr_C_S1, spr_C_S2, spr_C_S3, spr_C_S4, w_LT, sh_LG, sh_LC_S1, sh_LC_S2, sh_LC_S3, sh_LC_S4, KG_GOV_Y_ratio, KC_GOV_Y_ratio, KG_GOV, KC_GOV , IG_GOV, IC_GOV, IG_GOV_Y_ratio, IC_GOV_Y_ratio, KC_Y_ratio, KG_Y_ratio, KG_K_ratio, GVA_S1, GVA_S2, GVA_S3, GVA_S4, sh_EMIS_F_S1, sh_EMIS_F_S2, sh_EMIS_F_S3, sh_EMIS_F_S4, dd_S1, dd_S2, dd_S3, dd_S4, LG_L_ratio, LG_L_ratio_pseudo, tau_C, spr_1_pseudo, spr_2_pseudo, r_2_dCR_ddsr_max_pseudo, r_3_dCR_dCAR_max_pseudo, sh_GREEN_S1, sh_GREEN_S2, sh_GREEN_S3, sh_GREEN_S4, seq, EMIS_F_Y, TAX_C, SUB, gov_SUB, tau_C,tau_C_baseline, tau_C_policy_High, tau_C_policy_Medium, omega, EMIS_F_Y_SSP360, endog, KGE_PRI_S1, KGE_PRI_S2, KGE_PRI_S3, KGE_PRI_S4, KGE, KGNE_PRI_S1, KGNE_PRI_S2, KGNE_PRI_S3, KGNE_PRI_S4, KGNE, KCE_PRI_S1, KCE_PRI_S2, KCE_PRI_S3, KCE_PRI_S4, KCE, KCNE_PRI_S1, KCNE_PRI_S2, KCNE_PRI_S3, KCNE_PRI_S4, KCNE, KSEQ, KSEQ_PRI_S1, KSEQ_PRI_S2, seq, fiscal_balance_help, private_balance, firms_balance, households_balance, banks_balance, fiscal_balance_help_per, private_balance_per, firms_balance_per, households_balance_per, banks_balance_per, g_POP_stag, g_ucn, g_ucr, tucn, tucr, ucn, ucr, IG_E, IG_E_Y, IG_E_Y_cum, con_change, s_W, g_L_per), digits=8)))
  
  matrixname<-paste("Ecosystem",sce, sep="")
  assign (matrixname, (round(cbind(TEMP, CO2_CUM, dep_E, dep_M, REV_E, REV_M, D_T, hazratio, HW_CUM, EMIS, theta, M, REC, CEN, O2, MY, W, W_POP_ratio, DEM, EMIS_F, EMIS_L, g_EMIS_L, E, E_NF, E_F, ED, mu, rho, omega, epsilon, omega_ratio, mu_ratio, rho_ratio, epsilon_ratio, theta_per, haz_flow), digits=4)))
  
  matrixname<-paste("Variables",sce, sep="")
  assign (matrixname, (round(cbind(A, B, BAILOUT, B_C, b_C, B_CCB, b_CCB, B_CH, b_CH, B_G, b_G, B_GCB, b_GCB, B_GH, b_GH, BP, BP_D, BP_U, CO_GOV, CO_PRI, CO_PRI_N, CAP, CAR, CBP, CEN, CO2_CUM, CON_E, CON_M, coupon_C, coupon_G, CR, CR_C_S1, CR_C_S2, CR_C_S3, CR_C_S4, CR_G, D, DC, def, DEM, dep_E, dep_M, DL, DP, dsr, D_T, D_TF, D_TP, E, ED, EMIS, EMIS_F, EMIS_L, E_F, E_NF, GNS, g_EMIS_L, g_h, g_POP, gov_SUB, g_ucn, g_ucr, g_x_20, g_Y, g_beta_0, g_lambda, g_lambda_30, g_omega, h, hazratio, HPM, HW_CUM, I_GOV, I_PRI, IC_GOV,IC_PRI, IC_PRI_S1, IC_PRI_S2, IC_PRI_S3, IC_PRI_S4, IG_GOV, IG_PRI, IG_PRI_S1, IG_PRI_S2, IG_PRI_S3, IG_PRI_S4, I_PRI_D, I_PRI_D_S1, I_PRI_D_S2, I_PRI_D_S3, I_PRI_D_S4, IC_PRI_D_S1, IC_PRI_D_S2, IC_PRI_D_S3, IC_PRI_D_S4, IG_PRI_D_S1, IG_PRI_D_S2, IG_PRI_D_S3, IG_PRI_D_S4,  illiq, int_C_S1, int_C_S2, int_C_S3, int_C_S4, int_G, K, K_GOV, K_PRI, KC, KC_GOV, KC_PRI, KC_PRI_S1, KC_PRI_S2, KC_PRI_S3, KC_PRI_S4, KCE, KCE_PRI_S1, KCE_PRI_S2, KCE_PRI_S3, KCE_PRI_S4, KCNE, KCNE_PRI_S1, KCNE_PRI_S2, KCNE_PRI_S3, KCNE_PRI_S4, KG, KG_GOV, KG_PRI, KG_PRI_S1, KG_PRI_S2, KG_PRI_S3, KG_PRI_S4, KGE, KGE_PRI_S1, KGE_PRI_S2, KGE_PRI_S3, KGE_PRI_S4, KGNE, KGNE_PRI_S1, KGNE_PRI_S2, KGNE_PRI_S3, KGNE_PRI_S4, KSEQ, KSEQ_PRI_S1, KSEQ_PRI_S2, L, LC, LC_S1, LC_S2, LC_S3, LC_S4, LG, LG_S1, LG_S2, LG_S3, LG_S4, lev_B, LF, lf_1, M, MY, N, NLC_D_S1, NLC_D_S2, NLC_D_S3, NLC_D_S4, NLG_D_S1, NLG_D_S2, NLG_D_S3, NLG_D_S4, O2, p_C, p_G, POP, r, re, REC, RES_E, RES_M, REV_E, REV_M, RP, SEC, SEC_B, SEC_CB, SEC_H, seq, SES, sh_NLG, sh_NLC_S1, sh_NLC_S2, sh_NLC_S3, sh_NLC_S4, sh_L, sh_LC_S1, sh_LC_S2, sh_LC_S3, sh_LC_S4, sh_LG, spr, spr_G, spr_C_S1, spr_C_S2, spr_C_S3, spr_C_S4, s_W, SUB, TAX, TAX_C, TEMP, TAX_F, TAX_H, TP, TP_G, tucn, tucr, u, ucn, ucr, ue, um, ur, v, V_CB, V_H, V_HF, wage, W, w_C_S1, w_C_S2, w_C_S3, w_C_S4, w_G, w_LT, x_1, x_2, x_20, Y, Y_star, Y_star_E, Y_H, Y_HD, Y_HG,  yield_C, yield_G, Y_star_K, Y_star_M, Y_star_N, beta_S1, beta_S2, beta_S3, beta_S4, beta_0_S1, beta_0_S2, beta_0_S3, beta_0_S4, delta, epsilon, theta, kappa, lambda, lambda_30, mu, rho, sigma_0, tau_C, omega), digits=6)))
  
  matrixname<-paste("Parameters",sce, sep="")
  assign (matrixname, (round(cbind(ad_K, ad_LF, ad_P, c_1, c_2, CAR_min, con_E, con_M, CR_max, dd_S1, dd_S2, dd_S3, dd_S4, def_max, def_0, def_1, def_2, f_nc, gov_C, gov_IC, gov_IG, h_1, h_2, haz, int_A, int_D, int_S, l_1, lev_B_max, lf_2, p, p_C_bar, p_G_bar, pr, r_0, r_1, r_2, r_3, rep, s_B, s_C, s_F, s_G, s_W_0, s_W_1, sh_EMIS_F_S1, sh_EMIS_F_S2, sh_EMIS_F_S3, sh_EMIS_F_S4, sh_GREEN_S1, sh_GREEN_S2, sh_GREEN_S3, sh_GREEN_S4, sh_GVA_S1, sh_GVA_S2, sh_GVA_S3, sh_GVA_S4, spr_0, spr_1, spr_2, spr_3, TCRE, ur_T, w_H, w_S, x_10, x_11, x_21, alpha_00, alpha_01, alpha_1, alpha_2, alpha_31, alpha_32, alpha_41, alpha_42, alpha_51, alpha_52, beta_1, beta_2, gamma_E, gamma_E1, gamma_E2, gamma_E3, gamma_E4, gamma_SEQ1, gamma_SEQ2, delta_0, epsilon_max, epsilon_min, zeta_1, zeta_2, zeta_3, zeta_4, zeta_5, zeta_6, zeta_7, zeta_8, zeta_9, zeta_10, eta_1, eta_2, eta_3, lambda_10, lambda_10prime, lambda_11, lambda_12, lambda_13, lambda_14, lambda_15, lambda_20, lambda_20prime, lambda_21, lambda_22, lambda_23, lambda_24, lambda_25, lambda_30prime, lambda_31, lambda_32, lambda_33, lambda_34, lambda_35, lambda_40, lambda_40prime, lambda_41, lambda_42, lambda_43, lambda_44, lambda_45, mu_max, mu_min, xi, pi_1, pi_2, pi_3, pi_4, pi_5, pi_6, pi_7, pi_8, pi_9, pi_10, rho_max, sigma_1, sigma_2, tau_F, tau_H, phi), digits=8)))
...
xlabel1<-c(1, 10, 20, 30, 40, 50, 60, 70, 80)  
xlabel2<-c("2021", "2030","2040","2050","2060", "2070", "2080", "2090", "2100") 
...
Figures<-function(Figure,x2,x3,x4, Sce2, Sce3, Sce4, Colour4){
  ...
  plot(g_Y_per_mean1, type="l", col=Colour1, lwd=Width1, ylab="Growth rate of output (%)",  ylim=c(-1,6), yaxp=c(-1,6,7), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  ...
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(theta_per_mean1, type="l", col=Colour1, lwd=Width1, ylab=" Share of non-fossil energy (%)", ylim=c(10, 100), yaxp=c(10,100,9), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  ...
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(TEMP_mean1, type="l", col=Colour1, lwd=Width1, ylab=expression("Temperature change ("^{o}*"C)"), ylim=c(1,4), yaxp=c(1,4,6),  cex.lab=1.2, xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("TEMP_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("TEMP_mean",x3, sep="")))), col=Colour3,  pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("TEMP_mean",x4, sep="")))), col=Colour4,  pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(TEMP_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  ...
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(def_per_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Firms' default rate (%)", ylim=c(2,7), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  ...
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(spr_G_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Spread on green loans", ylim=c(0.02,0.06), yaxp=c(0.02,0.06,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  ...
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(spr_C_S1_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Spread on conv. loans, mining & utilities", ylim=c(0.02,0.06), yaxp=c(0.02,0.06,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  ...
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-13-Output.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(Y_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Output (trillion US$)", ylim=c(0, 800), yaxp=c(0,800,4),  xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("Y_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("Y_mean",x3, sep="")))), col=Colour3,  pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("Y_mean",x4, sep="")))), col=Colour4,  pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(Y_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-14-Output_per_capita.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(Y_POP_ratio_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Output per capita (thousand US$)", ylim=c(0, 125), yaxp=c(0,125, 5),  xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("Y_POP_ratio_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("Y_POP_ratio_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("Y_POP_ratio_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(Y_POP_ratio_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-15-Unemployment.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(ur_per_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Unemployment rate (%)", ylim=c(0,16), yaxp=c(0,16,8), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("ur_per_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2,cex=0.4) 
  lines(eval(parse(text=(paste("ur_per_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("ur_per_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(ur_per_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure 
  Cairo(file=paste("DEFINE-Fig",Figure,"-16-Debt_service_ratio.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(dsr_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Firms’ debt service ratio", ylim=c(0.3,0.9), yaxp=c(0.3,0.9,6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("dsr_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("dsr_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("dsr_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(dsr_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-17-Leverage_of_firms.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(L_K_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Firms’ leverage ratio", ylim=c(0,0.5), yaxp=c(0,0.5,10), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("L_K_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("L_K_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("L_K_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(L_K_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-18-Illiquidity_ratio.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(illiq_mean1, type="l", col=Colour1, lwd=Width1, ylab="Illiquidity ratio",  ylim=c(0.65,0.95), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("illiq_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("illiq_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("illiq_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(illiq_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-19-Total_credit_rationing.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(CR_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Total credit rationing", ylim=c(0,0.4), yaxp=c(0,0.4,8), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("CR_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("CR_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("CR_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(CR_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off()
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-20-Total_spread.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(spr_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Spread on total loans", ylim=c(0.02,0.06), yaxp=c(0.02,0.06,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("spr_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("spr_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("spr_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(spr_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off()
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-21-Public_debt_to_gdp.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(SEC_Y_per_mean1, type="l", col=Colour1, lwd=Width1, ylab="Public debt-to-output ratio (%)",  ylim=c(0,400), yaxp=c(0,400,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("SEC_Y_per_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("SEC_Y_per_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("SEC_Y_per_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(SEC_Y_per_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-22-Fiscal_balance.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(fiscal_balance_per_mean1, type="l", col=Colour1, lwd=Width1, ylab="Fiscal balance-to-output ratio (%)",  ylim=c(-16,0), yaxp=c(-16,0,8), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("fiscal_balance_per_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("fiscal_balance_per_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("fiscal_balance_per_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(fiscal_balance_per_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-23-Share_of_conventional_bonds.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(PORT_BCH_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Share of conventional bonds", ylim=c(0,0.2), yaxp=c(0,0.2,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("PORT_BCH_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("PORT_BCH_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("PORT_BCH_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(PORT_BCH_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-24-Share_of_green_bonds.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(PORT_BGH_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Share of green bonds", ylim=c(0,0.08), yaxp=c(0,0.08,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("PORT_BGH_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("PORT_BGH_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("PORT_BGH_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(PORT_BGH_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-25-Yield_on_conventional_bonds.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(yield_C_mean1, type="l", col=Colour1, lwd=Width1, ylab="Yield on conventional bonds",  ylim=c(0,0.3), yaxp=c(0,0.3,6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("yield_C_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("yield_C_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("yield_C_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(yield_C_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-26-Yield_on_green_bonds.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(yield_G_mean1, type="l", col=Colour1, lwd=Width1, ylab="Yield on green bonds",  ylim=c(0,0.2), yaxp=c(0,0.2,4), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("yield_G_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("yield_G_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("yield_G_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(yield_G_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure 
  Cairo(file=paste("DEFINE-Fig",Figure,"-27-Investment_ratio.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(I_K_ratio_mean1, type="l", col=Colour1, lwd=Width1, ylab="Investment ratio",  ylim=c(0,0.15), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("I_K_ratio_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("I_K_ratio_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("I_K_ratio_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(I_K_ratio_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-28-Share_of_desired_green.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(beta_S1_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Share of desired green inv., mining & utilities ", ylim=c(0,1), yaxp=c(0,1,5), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd = 0.5)   
  lines(eval(parse(text=(paste("beta_S1_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2,cex=0.4) 
  lines(eval(parse(text=(paste("beta_S1_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("beta_S1_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(beta_S1_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomright", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-29-Share_of_actual_green_investment.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(greenI_mean1, type="l", col=Colour1, lwd=Width1, ylab="Share of actual green investment",  ylim=c(0,1), yaxp=c(0,1,5), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("greenI_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2,cex=0.4) 
  lines(eval(parse(text=(paste("greenI_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("greenI_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(greenI_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-30-Share_of_green_capital.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(kappa_mean1, type="l", col=Colour1, lwd=Width1, ylab="Share of green capital",  ylim=c(0,0.3), yaxp=c(0,0.3,6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("kappa_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2,cex=0.4) 
  lines(eval(parse(text=(paste("kappa_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("kappa_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(kappa_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-31-Population.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(POP_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Population (billion)", ylim=c(6,14), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("POP_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("POP_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("POP_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(POP_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-32-Labour_force.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(LF_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Labour force (billion)", ylim=c(2,6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("LF_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("LF_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("LF_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(LF_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomright", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-33-Labour_force_to_population.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(lf_mean1, type="l", col=Colour1, lwd=Width1, ylab= "Labour force-to-population ratio", ylim=c(0.38,0.48), yaxp=c(0.38,0.48,5), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("lf_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  ...
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-34-Annual_working_hours.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(h_mean1, type="l", col=Colour1, lwd=Width1, ylab="Annual working hours",  ylim=c(1500,2000), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("h_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2 ,cex=0.4) 
  lines(eval(parse(text=(paste("h_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("h_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(h_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("bottomleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-35-Financial_balances.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(fiscal_balance_help_per_mean1, type="l", col=Colour1, lwd=Width1, ylab="Financial balances in % of GDP",  ylim=c(-35,40), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(firms_balance_per_mean1, col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4)
  lines(households_balance_per_mean1, col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4)
  lines(banks_balance_per_mean1, col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  legend("topleft", legend=c(Balance1, Balance2, Balance3, Balance4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), cex=0.6, bty = "n") 
  dev.off() 
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-36-Energy_Depletion_ratio.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(dep_E_mean1, type="l", col=Colour1, lwd=Width1, ylab="Energy depletion ratio",  ylim=c(0,0.05), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("dep_E_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("dep_E_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("dep_E_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(dep_E_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-37-Matter_depletion_ratio.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(dep_M_mean1, type="l", col=Colour1, lwd=Width1, ylab="Matter depletion ratio",  ylim=c(0,0.2), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("dep_M_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("dep_M_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("dep_M_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(dep_M_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-38-Co2_cumulative.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(CO2_CUM_mean1, type="l", col=Colour1, lwd=Width1, ylab=expression("Cumulative CO" [2]*" emissions (GtCO" [2]*")"),  ylim=c(0,7000), yaxp=c(0,7000,7), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("CO2_CUM_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("CO2_CUM_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3,cex=0.4) 
  lines(eval(parse(text=(paste("CO2_CUM_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(CO2_CUM_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-39-Recycling_rate.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(rho_mean1, type="l", col=Colour1, lwd=Width1, ylab=expression("Recycling rate"),  ylim=c(0,1), yaxp=c(0,1, 5), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("rho_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("rho_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("rho_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(rho_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-40-Matter_intensity.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(mu_mean1, type="l", col=Colour1, lwd=Width1, ylab=expression("Material intensity (kg/US$)"),  ylim=c(0.4,1.2), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("mu_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("mu_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("mu_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(mu_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-41-Energy_intensity.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(epsilon_mean1, type="l", col=Colour1, lwd=Width1, ylab=expression("Energy intensity (EJ/US$ trillion)"),  ylim=c(0,12), yaxp=c(0,12,6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("epsilon_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("epsilon_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("epsilon_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(epsilon_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-42-Waste.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(W_mean1, type="l", col=Colour1, lwd=Width1, ylab="Waste (Gt/year)",  ylim=c(0,100), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("W_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("W_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("W_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(W_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-43-Waste_per_capita.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(W_POP_ratio_mean1, type="l", col=Colour1, lwd=Width1, ylab="Waste per capita (tonnes/year)",  ylim=c(0,8), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd = 0.5)   
  lines(eval(parse(text=(paste("W_POP_ratio_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("W_POP_ratio_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("W_POP_ratio_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(W_POP_ratio_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-44-Haz_waste.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(hazratio_mean1, type="l", col=Colour1, lwd=Width1, ylab="Cum. haz. waste per capita (tonnes)",  ylim=c(0, 20), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(10, 80, by = 10),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("hazratio_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("hazratio_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("hazratio_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(hazratio_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off()
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-45-Wage_share.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(s_W_mean1, type="l", col=Colour1, lwd=Width1, ylab="Wage share",  ylim=c(0.5, 0.6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  lines(eval(parse(text=(paste("s_W_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("s_W_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("s_W_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(s_W_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topleft", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off()
  
  #Figure
  Cairo(file=paste("DEFINE-Fig",Figure,"-46-Growth_rate_of_loans.jpeg", sep=""), width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(g_L_per_mean1, type="l", col=Colour1, lwd=Width1, ylab="Growth rate of loans (%)",  ylim=c(-1,5), yaxp=c(-1,5,6), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(1,at = seq(3, 83, by = 5),tck = 1, lty = 3, col = "lightgray", lwd=0.5, labels=NA)
  axis(side=1, at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  grid(nx =NA, ny =NULL,   lty = 3, col = "lightgray",  lwd=0.5)   
  lines(eval(parse(text=(paste("g_L_per_mean",x2, sep="")))), col=Colour2, pch=pch2, type="o", lwd=Width2, cex=0.4) 
  lines(eval(parse(text=(paste("g_L_per_mean",x3, sep="")))), col=Colour3, pch=pch3, type="o", lwd=Width3, cex=0.4) 
  lines(eval(parse(text=(paste("g_L_per_mean",x4, sep="")))), col=Colour4, pch=pch4, type="o", lwd=Width4, cex=0.4) 
  lines(g_L_per_mean1, lty=Line1, col=Colour1, lwd=Width1)
  legend("topright", legend=c(Scenario1, Sce2, Sce3, Sce4), lty=c(Line1, Line2, Line3, Line4), col=c(Colour1, Colour2, Colour3, Colour4), pch=c(pch1, pch2, pch3, pch4), bty="n", cex=0.7) 
  dev.off() 
  
  #Figure
  Cairo(file="DEFINE-FigExtra-Carbon_tax.jpeg", width=width_graph, height=height_graph, res=res_graph)
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(tau_C_baseline_per, type="l", col=Colour1, lwd=Width1, ylab="Carbon tax ($/tCO2)",  ylim=c(0,600), xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
  lines(tau_C_policy_Medium_per, col=Colour2, pch=pch2, type="o", lwd=Width2,cex=0.7) 
  legend("topleft", legend=c(Carbon_Baseline, Carbon_M), lty=c(Line1, Line2), col=c(Colour1, Colour2), pch=c(pch1, pch2), cex=0.6, bty = "n") 
  #legend("topleft", legend=c(parse(text=sprintf('R^2'),R)), lty=c(Line1, Line2), col=c(Colour1, Colour2), pch=c(pch1, pch2), cex=0.6, bty = "n") 
  dev.off() 
  ...
  par(mar = c(3,3.3,1,1)+0.1,mgp=c(1.8,0.5,0), family=font)
  plot(r_total_mean2, type="l", col=Colour1, lwd=Width1, ylab="Firms' profit rate (%)",  ylim=c(4, 10), yaxp=c(4,10,6),  xlab=" ", xaxt="n", cex.lab=0.8, cex.axis=0.7)
  axis(side=1,at=xlabel1, labels=xlabel2, cex.lab=0.8, cex.axis=0.7)
...
if (figures_dummy==1) {
  Figures("1",4,5,6,"CT","CT+GS", "GPI", Colour4a)
  Figures("2",7,8,9,"DPF","GSF", "GQE", Colour4a)
  Figures("3",10,11,12,"Sufficiency","Fiscal+Financial","Sufficiency+Fiscal+Financial",Colour4a)
...
assign (matrixname, (round(rbind(c(g_Y_per_mean1[1], g_Y_per_mean1[30], mean(g_Y_per_mean1[1:30]), sd(g_Y_per_mean1[1:30])), c(ur_per_mean1[1], ur_per_mean1[30], mean(ur_per_mean1[1:30]), sd(ur_per_mean1[1:30])), c(POP_mean1[1], POP_mean1[30], mean(POP_mean1[1:30]), sd(POP_mean1[1:30])), c(theta_per_mean1[1], theta_per_mean1[30], mean(theta_per_mean1[1:30]), sd(theta_per_mean1[1:30])) , c(epsilon_ratio_mean1[1], epsilon_ratio_mean1[30], mean(epsilon_ratio_mean1[1:30]), sd(epsilon_ratio_mean1[1:30])) ,c(mu_ratio_mean1[1], mu_ratio_mean1[30], mean(mu_ratio_mean1[1:30]), sd(mu_ratio_mean1[1:30])) , c(EMIS_mean1[1], EMIS_mean1[30], mean(EMIS_mean1[1:30]), sd(EMIS_mean1[1:30])) ,c(tau_C_baseline[1]*1000, tau_C_baseline[30]*1000, mean(tau_C_baseline[1:30])*1000, sd(tau_C_baseline[1:30])*1000) , c(IG_E_Y_mean1[1], IG_E_Y_mean1[30], mean(IG_E_Y_mean1[1:30]), sd(IG_E_Y_mean1[1:30])) , c(def_per_mean1[1], def_per_mean1[30], mean(def_per_mean1[1:30]), sd(def_per_mean1[1:30])) , c(yield_C_per_mean1[1], yield_C_per_mean1[30], mean(yield_C_per_mean1[1:30]), sd(yield_C_per_mean1[1:30])) ,c(yield_G_per_mean1[1], yield_G_per_mean1[30], mean(yield_G_per_mean1[1:30]), sd(yield_G_per_mean1[1:30]))), digits=8)))
